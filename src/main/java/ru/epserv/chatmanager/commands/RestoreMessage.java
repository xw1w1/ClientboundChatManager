...
public class RestoreMessage {
    public static void register(CommandDispatcher<ServerCommandSource> dispatcher, CommandRegistryAccess commandRegistryAccess, CommandManager.RegistrationEnvironment registrationEnvironment) {
        dispatcher.register(CommandManager.literal("restore-message")
                .then(CommandManager.argument("id", integer()).executes(RestoreMessage::handle)));
    }
    ...
    private static int handle(CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException {
        ...
        if (message != null && message.isDeleted() && message.getID() == id) {
            ChatManager.getChatHistory().restoreMessage(id);
            return 1;
        } else {
            return -1;
        }
